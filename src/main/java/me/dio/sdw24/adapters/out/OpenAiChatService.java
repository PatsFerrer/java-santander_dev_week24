...
@FeignClient(name = "openAiChatApi", url = "${spring.openai.base-url}", configuration = OpenAiChatService.Config.class)
public interface OpenAiChatService extends GenerativeAiService {
    ...
    class Config {
        @Bean
        public RequestInterceptor apiKeyRequestInterceptor(@Value("${spring.openai.api-key}") String apiKey){
